...
Siga este passo a passo detalhado para colocar sua loja online agora mesmo!

---

## 📥 1. Baixando o Projeto
1. No menu do AI Studio, clique em **"Download Project"** (ícone de seta para baixo).
2. Você receberá um arquivo `.zip`.
3. **Extraia tudo:** Clique com o botão direito no arquivo e escolha "Extrair Tudo". Você terá uma pasta com todos os arquivos do site.

---

## ☁️ 2. Colocando no GitHub (Seu "Cofre" de Código)
1. Crie uma conta em [github.com](https://github.com).
2. Clique no botão verde **"New"** para criar um repositório.
3. Nome: `minha-vitrine-adidas`.
4. Deixe como **Public** e clique em **Create repository**.
5. Na tela que abrir, clique no link **"uploading an existing file"**.
6. **IMPORTANTE:** Arraste todos os arquivos da pasta do seu computador para o GitHub. 
   *   *Não arraste a pasta inteira, entre nela e selecione tudo.*
   *   *Certifique-se de que arquivos como `package.json` e `server.ts` apareçam na lista.*
7. Clique em **"Commit changes"** no final da página.

---

## 🌐 3. Deixando Online no Render.com
1. Crie uma conta em [render.com](https://render.com) (use sua conta do GitHub para entrar).
2. Clique em **"New +"** -> **"Web Service"**.
3. Conecte seu GitHub e escolha o repositório `minha-vitrine-adidas`.
4. Preencha exatamente assim:
   *   **Name:** O nome da sua loja (ex: `vitrine-adidas-joao`).
   *   **Runtime:** `Node`
...
## 💎 4. Como não perder os dados (Persistência)
No plano gratuito do Render, o banco de dados padrão (SQLite) reseta. Para evitar isso, siga estes passos para usar um banco de dados **PostgreSQL** gratuito e permanente:

1.  Crie uma conta no [Supabase](https://supabase.com) ou [Neon.tech](https://neon.tech).
2.  Crie um novo projeto e procure pela **"Connection String"** ou **"Database URL"**.
3.  Ela deve se parecer com isso: `postgresql://user:password@host:port/dbname`.
4.  No painel do **Render**, vá em **Environment** (Variáveis de Ambiente).
5.  Adicione uma nova variável:
    *   **Key:** `DATABASE_URL`
    *   **Value:** (Cole o link que você pegou no passo 2).
6.  Salve e o Render vai reiniciar o site. **Pronto!** Agora seus produtos nunca mais sumirão.

---

## ⚠️ 5. Aviso sobre Imagens
As imagens que você sobe no painel administrativo são salvas como texto (Base64) no banco de dados. 
*   **Vantagem:** Elas não somem se você usar o banco de dados permanente (Passo 4).
*   **Atenção:** Tente não subir fotos gigantescas (acima de 1MB), pois isso pode deixar o banco de dados lento.

---

## ⚙️ 6. Configuração Final
1. Acesse o link que o Render te deu (ex: `https://sua-loja.onrender.com`).
2. Vá em `/admin` (ex: `https://sua-loja.onrender.com/admin`).
3. Login: `admin` / Senha: `admin123`.
4. **MUITO IMPORTANTE:** Vá em **Configurações** e mude o número do WhatsApp para o SEU número.

---

## 📊 6. Capacidade
- **Acessos:** 50-100 pessoas ao mesmo tempo.
- **Velocidade:** O site é otimizado para celulares.

---
*Pronto para vender! Boas vendas com a Vitrine das Três Listras!*
...
if (process.env.DATABASE_URL) {
  console.log("🚀 Usando Banco de Dados Cloud (PostgreSQL)");
  pgPool = new pg.Pool({
  ...
  sqliteDb = new Database("catalog.db");
}
...
async function initDb() {
  ...
  const settings = await query("SELECT * FROM settings WHERE key = ?", ["whatsapp_number"]);
  if (settings.length === 0) {
    await query("INSERT INTO settings (key, value) VALUES (?, ?)", ["whatsapp_number", "5511999999999"]);
  }

  // Migration: Add columns if they don't exist (Postgres handles IF NOT EXISTS in column add differently, but we'll try simple approach)
  if (!isPg) {
    const tableInfo = await query("PRAGMA table_info(products)");
    const columns = (tableInfo as any[]).map(col => col.name);
    if (!columns.includes("sizes")) await exec("ALTER TABLE products ADD COLUMN sizes TEXT DEFAULT '{}'");
    if (!columns.includes("show_sizes")) await exec("ALTER TABLE products ADD COLUMN show_sizes INTEGER DEFAULT 0");
    if (!columns.includes("reference")) await exec("ALTER TABLE products ADD COLUMN reference TEXT");
  }
  ...
  const users = await query("SELECT * FROM users WHERE username = ?", [adminUser]);
  if (users.length === 0) {
    await query("INSERT INTO users (username, password) VALUES (?, ?)", [adminUser, adminPass]);
  }
}
...
async function startServer() {
  ...
  app.get("/api/health", (req, res) => {
    res.json({ status: "ok", database: pgPool ? "postgresql" : "sqlite" });
  });
  ...
  app.post("/api/products", async (req, res) => {
    const { name, description, price, image, sizes, show_sizes, reference } = req.body;
    const result = await query(
      "INSERT INTO products (name, description, price, image, sizes, show_sizes, reference) VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING id", 
      [name, description, price, image, JSON.stringify(sizes || {}), show_sizes ? 1 : 0, reference || null]
    );
    
    // Postgres returns RETURNING id, SQLite returns lastInsertRowid
    const id = pgPool ? result[0].id : (result as any).lastInsertRowid;
    res.json({ id });
  });
  ...
  app.put("/api/products/:id", async (req, res) => {
    ...
    const { name, description, price, image, sizes, show_sizes, reference } = req.body;
    await query(
      "UPDATE products SET name = ?, description = ?, price = ?, image = ?, sizes = ?, show_sizes = ?, reference = ? WHERE id = ?",
      [name, description, price, image, JSON.stringify(sizes || {}), show_sizes ? 1 : 0, reference || null, id]
    );
  ...
  app.post("/api/products/bulk", async (req, res) => {
    ...
    for (const item of products) {
      await query(
        "INSERT INTO products (name, description, price, image, sizes, show_sizes, reference) VALUES (?, ?, ?, ?, ?, ?, ?)",
        [item.name, item.description, item.price, item.image || null, JSON.stringify(item.sizes || {}), item.show_sizes ? 1 : 0, item.reference || null]
      );
    }
